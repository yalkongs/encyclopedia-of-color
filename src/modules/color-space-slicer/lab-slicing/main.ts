...
class LabSlicing {
  ...
  private draw() {
    ...
    ctx.fillText('+a* (red)', cx + R - 24, cy - 6); ctx.fillText('−a* (green)', cx - R + 28, cy - 6);
    ctx.fillText('+b* (yellow)', cx, cy - R + 12); ctx.fillText('−b* (blue)', cx, cy + R - 4);
    ctx.textAlign = 'left';

    // Side profile (Lab solid silhouette, max chroma bulges mid-L).
    ...
    ctx.fillText('L*', sx0 + sw / 2, sy0 + sh + 18);
    ctx.textAlign = 'left';

    ctx.fillStyle = theme.goldDeep; ctx.font = 'italic 14px "Cormorant Garamond", Georgia, serif';
    ctx.fillText(`L* = ${this.L} — the sRGB gamut at this lightness`, w * 0.08, h - 12);
  }
}
window.addEventListener('DOMContentLoaded', () => new LabSlicing());
