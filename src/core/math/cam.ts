...
// CAT16 (CIE 248:2022) — replaces CAT02 + HPE with a single adaptation matrix.
const M_CAT16: M3 = [
...
/*
 * CIECAM16 forward (CIE 248:2022). Identical correlate maths to CIECAM02 but the
 * single CAT16 matrix replaces CAT02 + the Hunt-Pointer-Estévez step.
 * Verified vs colour-science: XYZ[19.01,20,21.78], white[95.05,100,108.88],
 * L_A=318.31, Y_b=20, average → J≈41.73, C≈0.103, h≈217.1, M≈0.107, Q≈195.37.
 */
export function ciecam16(xyz: V3, xyzW: V3, LA: number, Yb: number, sur: Surround = SURROUND_AVERAGE): CAM02 {
  const Yw = xyzW[1];
  const rgb = mul3(M_CAT16, xyz);
  const rgbW = mul3(M_CAT16, xyzW);
  const D = Math.max(0, Math.min(1, sur.F * (1 - (1 / 3.6) * Math.exp((-LA - 42) / 92))));
  const Dr = rgb.map((c, i) => c * (D * (Yw / rgbW[i]) + 1 - D)) as V3;
  const DrW = rgbW.map((c) => c * (D * (Yw / c) + 1 - D)) as V3;
  ...
  const ra = Dr.map((c) => nonlin(FL * c)) as V3;
  const raW = DrW.map((c) => nonlin(FL * c)) as V3;
